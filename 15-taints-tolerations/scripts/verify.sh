...
# Check taint exists
tainted_node="$(kubectl --request-timeout=15s get nodes -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{range .spec.taints[*]}{.key}={.value}:{.effect}{","}{end}{"\n"}{end}' | awk -F'\t' '$2 ~ /Env=Production:NoSchedule/{print $1; exit}')"
...
if ! kubectl --request-timeout=15s get pod prod-pod -n default >/dev/null 2>&1; then
  echo "Pod 'prod-pod' not found in default namespace"
  exit 1
fi

pod_status="$(kubectl --request-timeout=15s get pod prod-pod -n default -o jsonpath='{.status.phase}')"
...
# Check toleration
pod_yaml="$(kubectl --request-timeout=15s get pod prod-pod -n default -o yaml)"
