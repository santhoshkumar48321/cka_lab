...
if ! kubectl --request-timeout=15s get clusterrole pipeline-deployer >/dev/null 2>&1; then
  echo "ClusterRole 'pipeline-deployer' not found"
  exit 1
fi

cr_yaml="$(kubectl --request-timeout=15s get clusterrole pipeline-deployer -o yaml)"
...
if ! kubectl --request-timeout=15s get rolebinding pipeline-deployer-binding -n app-squad >/dev/null 2>&1; then
  echo "RoleBinding 'pipeline-deployer-binding' not found in namespace 'app-squad'"
  exit 1
fi

rb_yaml="$(kubectl --request-timeout=15s get rolebinding pipeline-deployer-binding -n app-squad -o yaml)"
