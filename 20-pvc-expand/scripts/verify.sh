...
if ! kubectl --request-timeout=15s get pvc web-pvc -n frontend >/dev/null 2>&1; then
  echo "PVC 'web-pvc' not found in namespace 'frontend'"
  exit 1
fi

pvc_phase=$(kubectl --request-timeout=15s get pvc web-pvc -n frontend -o jsonpath='{.status.phase}')
if ! test "$pvc_phase" = "Bound"; then
  echo "PVC 'web-pvc' must be Bound, got: $pvc_phase"
  exit 1
fi

pvc_storage=$(kubectl --request-timeout=15s get pvc web-pvc -n frontend -o jsonpath='{.spec.resources.requests.storage}')
if ! test "$pvc_storage" = "250Mi"; then
  echo "PVC 'web-pvc' storage request must be 250Mi, got: $pvc_storage"
  exit 1
fi

pv_phase=$(kubectl --request-timeout=15s get pv web-pv -o jsonpath='{.status.phase}' 2>/dev/null || echo "not-found")
...
vol_mounts=$(kubectl --request-timeout=15s get deployment web-app -n frontend \
...
volumes=$(kubectl --request-timeout=15s get deployment web-app -n frontend \
