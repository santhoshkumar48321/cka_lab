...
if ! kubectl --request-timeout=15s get deployment myapp -n default >/dev/null 2>&1; then
  echo "Deployment myapp not found in default namespace"
  exit 1
fi

container_count=$(kubectl --request-timeout=15s get deployment myapp -n default \
...
if ! kubectl --request-timeout=15s get deployment myapp -n default -o yaml | grep -q 'name: logshipper'; then
  echo "Sidecar container 'logshipper' not found"
  exit 1
fi

if ! kubectl --request-timeout=15s get deployment myapp -n default \
...
logshipper_cmd=$(kubectl --request-timeout=15s get deployment myapp -n default \
...
missing=$(kubectl --request-timeout=15s get deployment myapp -n default \
