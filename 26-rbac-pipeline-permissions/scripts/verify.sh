...
if ! kubectl --request-timeout=15s get clusterrole deployment-manager >/dev/null 2>&1; then
  echo "ClusterRole 'deployment-manager' not found"
  exit 1
fi

cr_yaml="$(kubectl --request-timeout=15s get clusterrole deployment-manager -o yaml)"
...
if ! kubectl --request-timeout=15s get rolebinding deploy-bot-staging -n staging >/dev/null 2>&1; then
  echo "RoleBinding 'deploy-bot-staging' not found in namespace 'staging'"
  exit 1
fi

rb_yaml="$(kubectl --request-timeout=15s get rolebinding deploy-bot-staging -n staging -o yaml)"
...
result=$(kubectl --request-timeout=15s auth can-i create deployments \
