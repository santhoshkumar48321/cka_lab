...
# STATUS should show Bound and VOLUME should show web-pv
```
...
```
Add under `spec.template.spec` (at the same level as `containers`):
```yaml
volumes:
- name: web-storage
  persistentVolumeClaim:
    claimName: web-pvc
```
Add under `spec.template.spec.containers[0].volumeMounts`:
```yaml
volumeMounts:
- name: web-storage
...
# Note: storageClassName is 'manual'
